...
n <- 1000
x <- data.frame(key1 = sample(LETTERS, n, TRUE), 
                key2 = sample(LETTERS, n, TRUE), val = rep.int(1, n))
...
```{r, echo=TRUE, eval=TRUE}
n <- 10
x <- data.frame(key = sample(LETTERS[1:2], n, TRUE), val = rep.int(1, n))
z <- by(x, x$key, FUN = rbind)
unclass(z)
```
...
```{r, echo=TRUE, eval=TRUE}
x <- data.frame(key = sample(LETTERS[1:4], 5, TRUE), val = rep.int(1, 5))
y <- data.frame(key = sample(LETTERS[1:4], 3, TRUE), val = rep.int(1, 3))
match(x$key, y$key)
```
...
In the fastest version we avoid multiple sorting. 
```{r, echo=TRUE, eval=FALSE}
oidx <- order(dat$Date)
